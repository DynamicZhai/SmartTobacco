...
#define WATER_PUMP_PIN                      10                       //GPIO10控制电磁阀开关
#define WATER_PUMP_PIN_MODE                 WIFI_IOT_GPIO_DIR_OUT        //GPIO10输出状态
#define WATER_PUMP_POWER_PIN                5                      //GPIO5控制电磁阀电源
...
{
    uint8_t         Num;                    //GPIO 编号
    WifiIotGpioDir      Mode;                   //工作模式
    WifiIotGpioValue    Value;                  //GPIO值
}TS_IO_PARAM;
//------------------------------------------------------------------------------
// ADC配置句柄
typedef struct
...
/******************************************************************************/
{
    ...
    // GpioInit(Param->Num);  
    GpioInit();                               //初始化GPIO
    GpioSetDir(Param->Num, Param->Mode);                 //设置GPIO工作状态
    if(Param->Mode == WIFI_IOT_GPIO_DIR_OUT)
    {
        GpioSetOutputVal(Param->Num, Param->Value);      //输出状态，设置输出电平
    }
    else
    {
        GpioGetInputVal(Param->Num, &Param->Value);       //输入状态，读取电平
    }
    return 0;
}
/*
...
/******************************************************************************/
{
    ...
    // IoSetFunc(WATER_PUMP_PIN, WIFI_IOT_IO_FUNC_GPIO_10_PWM1_OUT);
...
/******************************************************************************/
{
    GpioSetOutputVal(WATER_PUMP_PIN, State);
    if(State == 1)
    GpioSetOutputVal(WATER_PUMP_PIN, 1);
    // PwmStart(WIFI_IOT_PWM_PORT_PWM1, PWM_FREQ_DIVITION / 10 * (10), PWM_FREQ_DIVITION);
    else
...
/******************************************************************************/
{
    GpioSetOutputVal(WATER_PUMP_POWER_PIN, State);
}
/*
...
extern uint8_t pump_ctrl_state;
extern uint8_t pump_power_state;
void SoilHumiCtrl_Task(void)
/******************************************************************************/  
{
    Soil_Init();                                            //硬件初始化
    ...
    while (1)
    {
        ...
        // printf("\r\n=======================================\r\n");
        // printf("\r\nSoil humi is %d\r\n",s_Soil.Humidity);
        // printf("\r\nwater pump power state is %d\r\n",pump_power_state);
        // printf("\r\nwater pump ctrl state is %d\r\n",pump_ctrl_state);
        // printf("\r\n=======================================\r\n");
