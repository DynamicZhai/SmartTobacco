...
/******************************************************************************/ 
{
    ...
    retval = hi_i2c_read(SHT40_I2C_IDX,(SHT40_ADDR<<1)|1,&data);
    if (retval != 0) {
        printf("I2cRead() failed, %0X!\n", retval);
        return retval;
    }
    // memcpy(buffer, data.receive_buf, data.receive_len);
    // (void)buffLen;
    return 0;
}
/*
...
/******************************************************************************/
{
    ...
    uint32_t retval = hi_i2c_write(SHT40_I2C_IDX,(SHT40_ADDR<<1)|0,&data);
    if (retval != 0) {
...
/******************************************************************************/ 
{
    ...
    uint8_t buffer[SHT40_STATUS_RESPONSE_MAX + 32] = { 0 };
...
/******************************************************************************/ 
{
    hi_io_set_func(HI_GPIO_IDX_14, HI_IO_FUNC_GPIO_14_I2C0_SCL);
    hi_io_set_func(HI_GPIO_IDX_13, HI_IO_FUNC_GPIO_13_I2C0_SDA);
...
/******************************************************************************/ 
{
    // hi_gpio_init();                                   //初始化散热风扇GPIO
    GpioInit();
    hi_io_set_func(FAN_PIN, HI_IO_FUNC_GPIO_8_PWM1_OUT); /* GPIO12 */
    // GpioSetDir(FAN_PIN, FAN_PIN_MODE);                   //设置GPIO工作状态
    // GpioSetOutputVal(FAN_PIN,WIFI_IOT_GPIO_VALUE0);           //关闭散热风扇
    PwmInit(WIFI_IOT_PWM_PORT_PWM1); //

    hi_io_set_func(HI_IO_NAME_GPIO_12, HI_IO_FUNC_GPIO_12_GPIO); /* GPIO12 */
    // hi_gpio_init(FAN_POWER_PIN);                             //初始化散热风扇电源GPIO
    GpioSetDir(FAN_POWER_PIN, FAN_POWER_PIN_MODE);       //设置GPIO工作状态
    GpioSetOutputVal(FAN_POWER_PIN,WIFI_IOT_GPIO_VALUE0);     //关闭散热风扇电源
}
/*
* 函数名称 : SHT40Task
* 功能描述 : 温湿度传感器任务
...
/******************************************************************************/ 
{
    ...
    while(1) 
    {
        // Set_Sht40_Environment_Temp_Alarm_Threshold(30);//温度阈值30℃
        SHT40_StartMeasure();
        usleep(20*1000);  
        ...
        case 0:
            GpioSetOutputVal(FAN_POWER_PIN, WIFI_IOT_GPIO_VALUE0);             //关闭散热风扇电源
            PwmStop(WIFI_IOT_PWM_PORT_PWM1);                   //关闭散热风扇
            break;
        case 1:
            GpioSetOutputVal(FAN_POWER_PIN, WIFI_IOT_GPIO_VALUE1);             //开启散热风扇电源
            PwmStart(WIFI_IOT_PWM_PORT_PWM1, PWM_FREQ_DIVITION * fan_level * 0.01, PWM_FREQ_DIVITION);
        ...
        //     {
        //         GpioSetOutputVal(FAN_POWER_PIN, WIFI_IOT_GPIO_VALUE1);             //开启散热风扇电源
        //         PwmStart(WIFI_IOT_PWM_PORT_PWM1, PWM_FREQ_DIVITION / 10 * (fan_level + 7), PWM_FREQ_DIVITION);                   //开启散热风扇
        //         // PwmStart(WIFI_IOT_PWM_PORT_PWM1, PWM_FREQ_DIVITION / 10 * (3 + 7), PWM_FREQ_DIVITION);
        //     }
        //     else
        //     {
        //         GpioSetOutputVal(FAN_POWER_PIN, WIFI_IOT_GPIO_VALUE0);             //关闭散热风扇电源
