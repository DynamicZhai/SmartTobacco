...
int mqtt_connect_conn1(void)
{
	...
	data.clientID.cstring = "mqtt_conn01A"; // 修改成自己名称，
	data.keepAliveInterval = 60;	// 心跳时间
	data.cleansession = 1;
	data.username.cstring = "tobacco_01_b";
	...
	while (1)
	{
		...
		if (MQTTPacket_read(buf, buflen, transport_getdata) == PUBLISH)
		{
			...
			//获取并打印 "light_mode" 属性的值
			cJSON *light_mode_json = cJSON_GetObjectItemCaseSensitive(json, "light_mode");
			if (cJSON_IsNumber(light_mode_json)) {
				int value = light_mode_json->valueint;
				light_mode = value;
				// printf("light_mode: %d\n", value);
			}
			// 获取并打印 "light_level" 属性的值
			cJSON *light_level_json = cJSON_GetObjectItemCaseSensitive(json, "light_level");
			if (cJSON_IsNumber(light_level_json)) {
				int value = light_level_json->valueint;
				light_level = value;
		...
		// printf("\nlight_intensity: %u\n", light_intensity);
		sprintf(payload, "{\"title\":1,\"light_intensity\":%d}", light_intensity);
